...
class dbPtw {
     ...
     * #__  Worptwess prefix
     * ^__  Store plugin tables prefix (@see PTW_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, PTW_DB_PREF, $wpdb->prefix. PTW_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampPtw($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, PTW_DATE_DL)) return dateToTimestampPtw($date);
        $arr = explode('-', $date);
        return dateToTimestampPtw($arr[2]. PTW_DATE_DL. $arr[1]. PTW_DATE_DL. $arr[0]);
    }
