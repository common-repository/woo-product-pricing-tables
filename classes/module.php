<?php
abstract class modulePtw extends baseObjectPtw {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsPtw::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsPtw::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return PTW_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return PTW_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importPtw($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNamePtw($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassPtw($helper, $this->getModDir(). 'helper.php');
