...
abstract class tablePtw {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(PTW_CODE);
    ...
    public function addJoin($join/*$params = array('tbl' => '', 'a' => '', 'on' => '', 'joinOnID' => true, 'joinOn' => '')*/) {
		...
		$this->_join[] = $join;	// As easy as possible
        return $this;
    }
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjPtw('fieldPtw', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbPtw::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbPtw::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbPtw::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothig to update', PTW_LANG_CODE));
            return false;
        }
        ...
        if(dbPtw::query($query)) {
            if($method == 'INSERT')
                return dbPtw::lastID();
            else
                return true;
        } else
			$this->_addError(PTW_TEST_MODE ? dbPtw::getError() : __('Database error. Please contact your developer.', PTW_LANG_CODE));
        return false;
    }
    ...
    /**
     * Add new fieldPtwPtw for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjPtw('fieldPtw', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
	/**
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbPtw::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(PTW_DATE_FORMAT, dbPtw::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbPtw::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
