<?php
abstract class viewPtw extends baseObjectPtw {
    /* 
     * @deprecated
     */
    protected $_tpl = PTW_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = framePtw::_()->getModule( $this->_code );
		$plTemplate = framePtw::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !framePtw::_()->getModule($plTemplate))
			$plTemplate = '';
	...
	public function getModel($code = '') {
		return framePtw::_()->getModule( $this->_code )->getController()->getModel($code);
	}
