...
class htmlPtw {
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = PTW_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function selectbox($name, $params = array('attrs' => '', 'options' => array(), 'value' => '')) {
        ...
        if(!empty($params['options'])) {
            foreach($params['options'] as $k => $v) {
                $selected = (isset($params['value']) && $k == $params['value'] ? 'selected="selected"' : '');
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		framePtw::_()->addScript('ajaxupload', PTW_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. PTW_CODE) === false)
			$params['url'] = uriPtw::_(array('baseUrl' => $params['url'], 'pl' => PTW_CODE));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterPtw::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsPtw::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterPtw::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlPtw::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsPtw::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlPtw::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = framePtw::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			foreach(array_merge(utilsPtw::getFontsList(), utilsPtw::getStandardFontsList()) as $font)
				$options[ $font ] = $font;
		}
		$params['options'] = array();
		if(isset($params['default'])) {
			$params['options'] = array(PTW_DEFAULT => $params['default']);
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellPtw" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtPtw"></span>
			<span class="toeSlideOnPtw">'. __('ON'). '</span>
			<span class="toeSlideOffPtw">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', PTW_LANG_CODE), strFirstUp($galleryType));
	...
	/*static public function radialProgress($progress, $attrs = array()) {
		framePtw::_()->addStyle('radial-progress', PTW_CSS_PATH. 'radial-progress.css');
