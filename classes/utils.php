...
class utilsPtw {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsPtw::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsPtw::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!framePtw::_()->getModule($m))
                return false;
            if(!is_dir(framePtw::_()->getModule($m)->getModDir()))
                return false;
        }
        return true;*/
        foreach(framePtw::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqPtw::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worptwess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getStandardFontsList() {
