...
class fieldPtw {
	 ...
	 */
	public $adapt = array('htmlPtw' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterPtw::_($value, $this->adapt['dbFrom'], fieldAdapterPtw::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlPtw', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterPtw::_($this, $this->adapt['html'], fieldAdapterPtw::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = framePtw::_()->getModule('optionsPtw')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', PTW_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsPtw'] = $options;
				}
			}
			$htmlContent = htmlPtw::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlPtw::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterPtw::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterPtw::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsPtw');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = framePtw::_()->getModule('optionsPtw')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsPtw'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlPtw', $method)) {
			$output .= htmlPtw::$method($name, $params);
			$output .= htmlPtw::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', PTW_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', PTW_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', PTW_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterPtw::_($id,'getExtraFieldOptions',fieldAdapterPtw::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', PTW_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', PTW_LANG_CODE).':<br />';
					$output .= htmlPtw::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', PTW_LANG_CODE).':<br />';
					$output .= htmlPtw::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterPtw::_($this, $this->adapt['htmlParams'], fieldAdapterPtw::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', PTW_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', PTW_LANG_CODE).':<br />';
				$output .= fieldAdapterPtw::_($params,'getFieldAttributes',  fieldAdapterPtw::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsPtw::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperPtw');
		  $module = $this->checkVarFromParam($param,'modulePtw');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!framePtw::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(framePtw::_()->getModule($module)->isExternal())
		   $config_xml = framePtw::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = PTW_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', PTW_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = framePtw::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsPtw'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlPtw::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
