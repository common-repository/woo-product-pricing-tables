...
abstract class controllerPtw {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = framePtw::_()->getModule( $this->getCode() );
		$className = '';
		if(importPtw($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNamePtw($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importPtw($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNamePtw($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(PTW_ADMIN),
				'remove' => array(PTW_ADMIN),
				'restore' => PTW_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - PTW_METHODS or PTW_USERLEVELS
	 */
	public function getPermissions() {
		return array();
	}
	public function getModule() {
		return framePtw::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqPtw::getVar('page');
		$rowsLimit = (int) reqPtw::getVar('rows');
		$orderBy = reqPtw::getVar('sidx');
		$sortOrder = reqPtw::getVar('sord');

		// Our custom search
		$search = reqPtw::getVar('search');
		...
		// jqGrid search
		$isSearch = reqPtw::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqPtw::getVar('searchField'));
			$searchString = trim(reqPtw::getVar('searchString'));
		...
		$res = dispatcherPtw::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec(true);
	}
	public function removeGroup() {
		$res = new responsePtw();
		if($this->getModel()->removeGroup(reqPtw::getVar('listIds', 'post'))) {
