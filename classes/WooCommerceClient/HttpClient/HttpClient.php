...
class ptwWccHttpClient
{
    ...
    public function __construct($url, $consumerKey, $consumerSecret, $options)
    {
        if (!\function_exists('curl_version')) {
            throw new ptwWccHttpClientException('cURL is NOT installed on this server', -1, new ptwWccRequest(), new ptwWccResponse());
        }

        $this->options        = new ptwWccOptions($options);
    ...
    protected function getRequestHeaders($sendData = false)
    {
        $headers = [
            ...
            'User-Agent' => $this->options->userAgent() . '/' . ptwWccClient::VERSION,
        ];
    ...
    protected function createRequest($endpoint, $method, $data = [], $parameters = [])
    {
        ...
        $this->request = new ptwWccRequest(
    ...
    protected function createResponse()
    {
        ...
        // Register response.
        $this->response = new ptwWccResponse($code, $headers, $body);
    ...
    protected function lookForErrors($parsedResponse)
    {
        ...
        if (!\in_array($this->response->getCode(), ['200', '201', '202'])) {
            ...
            throw new ptwWccHttpClientException(
    ...
    protected function processResponse()
    {
        ...
        if (JSON_ERROR_NONE !== json_last_error()) {
            ...
            throw new ptwWccHttpClientException($message, $this->response->getCode(), $this->request, $this->response);
        }
    ...
    public function request($endpoint, $method, $data = [], $parameters = [])
    {
        ...
        if (\curl_errno($this->ch)) {
            throw new ptwWccHttpClientException('cURL Error: ' . \curl_error($this->ch), 0, $request, $response);
        }
