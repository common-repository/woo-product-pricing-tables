...
class responsePtw {
    ...
    public function ajaxExec($forceAjax = false) {
        $isAjax = $this->isAjax();
        $redirect = reqPtw::getVar('redirect');
        if(count($this->errors) > 0)
            $this->error = true;
        if($isAjax || $forceAjax)
            exit( json_encode($this) ); 
        /*if($redirect)
            redirectPtw($redirect);*/
        return $this;
    }
	public function mainRedirect($redirectUrl = '') {
		$redirectUrl = empty($redirectUrl) ? PTW_SITE_URL : $redirectUrl;
		$redirectData = array();
		if(!empty($this->errors)) {
			$redirectData['ptwErrors'] = $this->errors;
		}
		if(!empty($this->messages)) {
			$redirectData['ptwMsgs'] = $this->messages;
		}
        return redirectPtw($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
    ...
    /**
     * Alias for responsePtw::addError, @see addError method
     */
