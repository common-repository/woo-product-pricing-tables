 ...
 */
if(!function_exists('dateToTimestampPtw')) {
    function dateToTimestampPtw($date) {
        if(empty($a))
            return false;
        $a = explode(PTW_DATE_DL, $date);
...
if(!function_exists('importPtw')) {
    function importPtw($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassPtw')) {
    function importClassPtw($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), PTW_CODE) !== false) {
					$classFile = preg_replace('/'. PTW_CODE. '/i', '', $classFile);
				}
                $path = PTW_CLASSES_DIR. $classFile. '.php';
			}
            return importPtw($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, PTW_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], PTW_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNamePtw')) {
    function toeGetClassNamePtw($class) {
        $className = '';
		if(class_exists($class. strFirstUp(PTW_CODE)))
			$className = $class. strFirstUp(PTW_CODE);
		else if(class_exists(PTW_CLASS_PREFIX. $class))
            $className = PTW_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjPtw')) {
    function toeCreateObjPtw($class, $params) {
        $className = toeGetClassNamePtw($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectPtw')) {
    function redirectPtw($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsPtw')) {
	function prepareParamsPtw(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsPtw::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodePtw($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodePtw')) {
	function prepareFieldCodePtw($string) {
...
if(!function_exists('print_r2')) {
	function print_r2($obj1, $someText = '') {
		echo "$someText<pre>";
		print_r($obj1);
		echo "</pre>";
	}
