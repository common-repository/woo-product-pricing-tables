...
,	showSlidesEditWnd: function(block) {
		...
		if(!this.slidesEditWnd) {
			this.slidesEditWnd = jQuery('#ptwManageSlidesWnd').modal({
				show: false
			});
			this.slidesEditWnd.find('.ptwManageSlidesSaveBtn').click(function(){
				block.beforeSave();
				var listPrev = self.slidesEditWnd.find('.ptwSlidesListPrev')
				,	slides = block.getSlides()
				,	sliderShell = block.getSliderShell()
				,	tmpDiv = jQuery('<div style="display: none;" />').appendTo('body');
				listPrev.find('.ptwSlideManageItem').each(function(){
			...
			this.slidesEditWnd.find('.ptwSlideManageAddBtn').click(function(){
		...
		var listPrev = this.slidesEditWnd.find('.ptwSlidesListPrev');
		listPrev.find('*:not(.ptwSlideManageAddBtn)').remove();
		var slides = block.getSlides();
		if(slides && slides.size()) {
			slides.each(function(){
				var newItem = jQuery('#ptwSlideManageItemExl').clone().removeAttr('id');
				newItem.find('img:first').attr('src', jQuery(this).find('.ptwSlideImg').attr('src'));
				newItem.data('slide-id', jQuery(this).data('slide-id'));
				listPrev.prepend( newItem );
				newItem.find('.ptwSlideManageItemRemove').click(function(){
					//if(confirm(toeLangPtw('Are you sure want to remove this slide?'))) {
						jQuery(this).parents('.ptwSlideManageItem:first').hide(g_ptwAnimationSpeed, function(){
							jQuery(this).remove();
						});
					//}
					return false;
			...
			listPrev.sortable({
				revert: true
			,	items: '.ptwSlideManageItem'
			,	placeholder: 'ui-state-highlight'
			//,	axis: 'x'
			});
			listPrev.find('*').disableSelection();
		} else {
			listPrev.prepend( '<div>'+ toeLangPtw('You have no slides for now - try to add them at first.')+ '</div>' );
		}
		this.slidesEditWnd.modal('show');
	}
,	_getEllIconsLibHtml: function() {
		return this.iconsLibWnd.find('.ptwIconsLibList .ptwIconLibItem');
	}
...
,	initIconsLibWnd: function() {
		var self = this;
		this.iconsLibWnd = jQuery('#ptwIconsLibWnd').modal({
			show: false
		});
		this.iconsLibWnd.find('.ptwIconsLibSearchTxt').keyup(function(){
		...
		this.iconsLibWnd.find('.ptwIconsLibSaveBtn').click(function(){
			ptwUtils.iconsLibWnd.modal('hide');
			return false;
		});
		var allIcons = this.getFaIconsList()
		,	iconsShell = this.iconsLibWnd.find('.ptwIconsLibList');
		iconsShell.html('');
		for(var i = 0; i < allIcons.length; i++) {
			var iconName = this._faIconClassToName(allIcons[i]);
			iconsShell.append('<div class="ptwIconLibItem col-md-3 col-sm-4" onclick="ptwUtils.selectFaIconFromLib(this); return false;" data-icon="'+ allIcons[i]+ '" data-name="'+ iconName+ '">'
				+ '<i class="ptwIconLibPrev fa '+ allIcons[i]+ '"></i>'
				+ '<span class="ptwIconLibTitle">'+ iconName+ '</span>'
			+'</div>');
		}
	}
...
,	_showNothingFoundIconsLib: function(keys) {
		var msgEl = this.iconsLibWnd.find('.ptwIconsLibEmptySearch');
		if(keys) {
			msgEl.find('.ptwNothingFoundKeys').html( keys );
		}
		msgEl.slideDown( g_ptwAnimationSpeed );
	}
,	_hideNothingFoundIconsLib: function() {
		this.iconsLibWnd.find('.ptwIconsLibEmptySearch').hide();
	}
,	showIconsLibWnd: function(element) {
		...
		this.iconsLibWnd.find('.ptwIconsLibSearchTxt').val('');
...
,	initBadgesLibWnd: function(tableColumn) {
		var self = this;
		this.badgesLibWnd = jQuery('#ptwBadgesLibWnd').modal({
			show: false
		});
		this.badgesLibWnd.find('.ptwBadgesLibSaveBtn').click(function(){
			self.badgesLibWndElement._setBadge( self.getBadgesData() );
			ptwUtils.badgesLibWnd.modal('hide');
			return false;
		});
		...
		for(var i = 0; i < colorInputs.length; i++) {
			inpSelector = '.ptwColorPickInput[name="' + colorInputs[i].key + '"]';
			var colorInp = this.badgesLibWnd.find(inpSelector);
			oneColorPickerOpt.altField = inpSelector + ' + .ptwColorPickInputTear';
			oneColorPickerOpt.position.of = inpSelector + ' + .ptwColorPickInputTear';
			colorInp.colorpicker(oneColorPickerOpt);
		}
		this.fillInBadgeSettings(tableColumn);
		this.badgesLibWnd.find('.ptwTableBadgePosition').click(function(){
			self.badgesLibWnd.find('.ptwTableBadgePosition').removeClass('active');
...
,	fillInBadgeSettings: function(tableColumn) {
		// init color picker
		var $form = jQuery('#ptwBadgesLibForm'),
			backgroundColor = '#444444',
			foregroundColor = '#ffffff';

		if(tableColumn) {
			var $badge = tableColumn._$.find('.ptwColBadgeContent');
			// badge position
		...
		for(var i = 0; i < colorInputs.length; i++) {
			this.badgesLibWnd.find('.ptwColorPickInput[name='+ colorInputs[ i ].key+ ']')
...
,	fillInBadgeLibData: function(data) {
		...
		if(data.badge_bg_color) {
			this.badgesLibWnd.find('.ptwColorPickInput[name=badge_bg_color]').colorpicker('setColor',  data.badge_bg_color );
		}
		if(data.badge_txt_color) {
			this.badgesLibWnd.find('.ptwColorPickInput[name=badge_txt_color]').colorpicker('setColor',  data.badge_txt_color );
		}
		if(data.badge_pos) {
			this.badgesLibWnd.find('.ptwTableBadgePosition[data-pos="'+ data.badge_pos+ '"]').click();
		}
	}
,	updateBadgePrevLib: function($badge, data) {
		$badge = $badge ? $badge : jQuery('#ptwTableBadgePrev');
		data = data ? data : this.getBadgesData();
		var $prevContent = $badge.find('.ptwColBadgeContent');
		$badge
			.attr({
				'class': 'ptwColBadge ptwColBadge-'+ data.badge_pos
			,	'style': ''
			});
...
,	getBadgesData: function() {
		var data = this.badgesLibWnd.find('#ptwBadgesLibForm').serializeAssoc();
		return data;
	}
};