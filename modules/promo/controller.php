<?php
class promoControllerPtw extends controllerPtw {
    public function welcomePageSaveInfo() {
		$res = new responsePtw();
		installerPtw::setUsed();
		if($this->getModel()->welcomePageSaveInfo(reqPtw::get('get'))) {
			$res->addMessage(__('Information was saved. Thank you!', PTW_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
		$originalPage = reqPtw::getVar('original_page');
		$http = isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) ? 'https://' : 'http://';
		if(strpos($originalPage, $http. $_SERVER['HTTP_HOST']) !== 0) {
			$originalPage = '';
		}
		redirectPtw($originalPage);
	}
	public function sendContact() {
		$res = new responsePtw();
		$time = time();
		$prevSendTime = (int) get_option(PTW_CODE. '_last__time_contact_send');
			...
			if($validate) {
				...
				foreach($validate as $v) {
					...
					switch($v) {
						case 'notEmpty':
							if(empty($data[ $fName ])) {
								$error = $fData['html'] == 'selectbox' ? __('Please select %s', PTW_LANG_CODE) : __('Please enter %s', PTW_LANG_CODE);
								$error = sprintf($error, $fData['label']);
							}
							break;
						case 'email':
							if(!is_email($data[ $fName ])) 
								$error = __('Please enter valid email address', PTW_LANG_CODE);
							break;
					}
					if(!empty($error)) {
		...
		if(!$res->error()) {
			...
			$msg .= 'Plugin: '. PTW_WP_PLUGIN_NAME. '<br />';
			foreach($fields as $fName => $fData) {
			...
			if(framePtw::_()->getModule('mail')->send('support@supsystic.zendesk.com', $data['subject'], $msg, $data['name'], $data['email'])) {
				update_option(PTW_CODE. '_last__time_contact_send', $time);
			} else {
				$res->pushError( framePtw::_()->getModule('mail')->getMailErrors() );
			}
	...
	public function dropTinyMceFullPlugins() {
		$plugsDir = PTW_JS_DIR. 'tinymce'. DS. 'plugins'. DS;
	...
	public function compileTinyMce() {
		...
		file_put_contents($putTo, str_replace('/*PTWO EXPORT HERE*/', $content, file_get_contents($putTo)));
		exit();
	}
	...
	public function addNoticeAction() {
		$res = new responsePtw();
		$code = reqPtw::getVar('code', 'post');
		$choice = reqPtw::getVar('choice', 'post');
		if(!empty($code) && !empty($choice)) {
			$optModel = framePtw::_()->getModule('options')->getModel();
			switch($choice) {
				case 'hide':
	...
	public function getPermissions() {
		return array(
			PTW_USERLEVELS => array(
				PTW_ADMIN => array('welcomePageSaveInfo', 'sendContact', 'compileTyniMce', 'dropTinyMceFullPlugins', 'addNoticeAction')
			),
		);
	}
}