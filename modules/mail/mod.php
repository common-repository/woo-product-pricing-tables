<?php
class mailPtw extends modulePtw {
	public function init() {
		parent::init();
		//dispatcherPtw::addFilter('optionsDefine', array($this, 'addOptions'));
	}
	...
	public function addOptions($optw) {
		$optw[ $this->getCode() ] = array(
			'label' => __('Mail', PTW_LANG_CODE),
			'optw' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', PTW_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', PTW_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', PTW_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $optw;
	}
