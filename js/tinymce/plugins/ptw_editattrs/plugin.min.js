tinymce.PluginManager.add('ptw_editattrs', function(editor) {
    var allowedAttr = ['id', 'class', 'style'];

    function getHtmlPanel() {
        var $newEl = jQuery('#ptwMceSubMenyAddHtmlAttr');
        $newMainEl = $newEl.clone().removeAttr('id');
        return $newMainEl[0].outerHTML;
    }

    editor.addButton('ptw_editattrs', {
        type: 'panelbutton'
        ,	icon: 'fullpage'
        ...
        ,	panel: {
            ...
            ,	onshow: function (e) {
                ...
                for(var i = 0; i < allowedAttr.length; i++) {
                    var input = control.find('label.ptwElMenuSubPanelRow[data-id=' + allowedAttr[i] + '] input');
