...
jQuery.fn.serializeAnythingPtw = function(addData) {
    var toReturn    = [];
...
function toeListablePtw(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.ptwExample, .ptwTblHeader').remove();
		var exampleRow = this.table.find('.ptwExample');
		for(var i in list) {
			...
			newRow.removeClass('ptwExample').show();
...
function setCookiePtw(c_name, value, exdays) {
	var exdate = new Date();
...
/**
 * Retrive worptwess attach ID from image, using img classes
...
function ptwCallWpMedia(params) {
	...
	wp.media.editor.send.attachment = function(optw, attach) {
		wp.media.editor._attachSent = true;
		var imgUrl = optw.size && attach.sizes[ optw.size ] && attach.sizes[ optw.size ].url
			? attach.sizes[ optw.size ].url
			: attach.url;
		params.clb( optw, attach, imgUrl );
	};
...
function ptwMceMoveToolbar(editor, clientX) {
	/*if(editor._ptwLastOpenedMenu) {
		editor._ptwHidingForMove = true;
		editor._ptwLastOpenedMenu.hide();
		editor._ptwLastOpenedMenu = null;
	}*/
	...
	panel._ptwOriginalTop = newY
}
/*function ptwMceOnShowMoveMainToolbar(e, editor) {
	var panel = editor.theme.panel
	,	prevRect = panel.layoutRect();
	if(typeof(panel._ptwOriginalTop) === 'undefined') {
		panel._ptwOriginalTop = prevRect.y;
	}
	// If we showld move it- but it is not in original position	- move it back
	if(prevRect.y != panel._ptwOriginalTop) {
		panel.ptwSetCanMove();
		panel.moveTo(prevRect.x, panel._ptwOriginalTop);
		panel._ptwRefreshedOriginal = true;
		//panel._ptwOriginalTop = prevRect.y;
	}
	var panelRect = panel.layoutRect()
...
function ptwMceOnHideMoveMainToolbar(e, editor) {
	var panel = editor.theme.panel;
	if(!editor._ptwHidingForMove && !panel._ptwRefreshedOriginal) {
		...
		ptwMceMoveToolbar(editor, newX);
	}
	editor._ptwHidingForMove = false;
	panel._ptwRefreshedOriginal = false;
	return;
	var panel = editor.theme.panel
...
function ptwMceOnShowSubMenu(e, editor) {
	...
	,	controlHeight = jQuery('#'+ e.control._id).height()
	,	newX = rootRect.x
	,	newY = (panel._ptwOriginalTop ? panel._ptwOriginalTop : rootRect.y) - controlHeight + 10;
	
	if(panelOnBottom) {
		newY += controlHeight + 20;
		jQuery('#'+ e.control._id).addClass('mce-tinymce-subpanel-bottom');
	}
	e.control.ptwSetCanMove();
	...
	editor._ptwLastOpenedMenu = e.control;
}*/
...
function ptwChangeElAttrs(scope) {
	this._scope = scope;
...
ptwChangeElAttrs.prototype._initMenuItem = function() {
	var self = this._scope,
		$parent = self._$.find('.ptwElMenuMainPanel'),
		$newEl = jQuery('#ptwAddHtmlAttribute');

	$newEl = $newEl
		.clone()
		.removeAttr('id')
		.addClass('ptwElMenuBtn');
	$parent.prepend('<div class="ptwElMenuBtnDelimiter"></div>');
	$parent.prepend($newEl);
};
ptwChangeElAttrs.prototype._initSubMenuItem = function() {
	var self = this._scope,
		$parent = self._$.find('.ptwElMenuContent'),
		$newEl = jQuery('#ptwSubMenyAddHtmlAttr');
...
ptwChangeElAttrs.prototype._getAttrToSubMenu = function () {
	var me = this,
		self = me._scope,
		$el = self._element._$.hasClass('ptwEl')
			? self._element._$
			: self._element._$.find('.ptwEl').eq(0),
	...
	$parent.find('.ptwElMenuSubPanel[data-sub-panel="add_attr"] .ptwElMenuSubPanelRow').remove();

	$el.each(function(){
		$attrNames = this.attributes;
		$parent = $parent.find('.ptwElMenuSubPanel[data-sub-panel="add_attr"]');
		...
		for (var key in allowedAttr) {
			...
			if(typeof allowedAttr[key] == 'string') {
				...
				$newEl = jQuery('#ptwRowAddHtmlAttr')
					.clone()
					.show()
					.removeAttr('id');
...
ptwChangeElAttrs.prototype._updateElAttrs = function() {
	this._isChangedAttrText = true;
};
ptwChangeElAttrs.prototype._setEventChangeElVal = function() {
	var me = this,
		$parent = this._itemSubMenu;
	...
	$parent.find('input:not(".ptwCpInpFlag")').unbind('keyup');
	$parent.find('input:not(".ptwCpInpFlag")').on('keyup', callBackFn);
};
ptwChangeElAttrs.prototype._setEventUpdateAttrs = function() {
	var me = this,
		$parent = this._itemSubMenu;
